...
/**
 * Created by yukh on 2016/11/15
 */
public class AnalyzeWebRiskBolt extends AbstractRedisBolt {
    private static final Logger logger = LoggerFactory.getLogger(AnalyzeWebRiskBolt.class);
    ...
    final String[] CT_CATE_EN = { "brand", "product1", "people", "other" };
    final String[] SYMBOL = { "(", ")", "（", "）", "《", "》" };

    public AnalyzeWebRiskBolt(JedisPoolConfig config) {
        super(config);
    }
...
//				}
//			}
            double dna_total_risk = 0;
            double dna_max_risk = 0;
            if (desRiskScore.size() > 0) {
                dna_max_risk = desRiskScore.get(0).getDna_score();
                for (DescRiskScore drs : desRiskScore)
                    dna_total_risk += drs.getDna_score();
            }
            List<ClientScore> nna_clients = new ArrayList<>();
...
//			irv.riskDebugInfo += sb.toString();
            String sna_clientDebugInfo2 = gson.toJson(cliDebugInfo);
            String sna_riskDebugInfo = sb.toString();

//			logger.info("~~~~~~~~~~~~~~~~~~" + gson.toJson(irv));

            source.put("dna_max_risk", dna_max_risk);
            source.put("dna_total_risk", dna_total_risk);
            source.put("ina_risk_version", 3);
            source.put("nna_risks", Entity.getMap(desRiskScore));
            if (sna_riskDebugInfo.length() > 0)
                source.put("sna_riskDebugInfo", sna_riskDebugInfo);
            if (sna_clientDebugInfo2.length() > 0)
                source.put("sna_clientDebugInfo2", sna_clientDebugInfo2);
            source.put("nna_clients", Entity.getMap(nna_clients));
