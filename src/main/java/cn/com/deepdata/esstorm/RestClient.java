...
public class RestClient implements Closeable, StatsAware {
	...
	public RestClient(Settings settings) {
		...
		if (ConfigurationOptions.ES_BATCH_WRITE_RETRY_POLICY_SIMPLE
				.equals(retryPolicyName)) {
			retryPolicyName = SimpleHttpRetryPolicy.class.getName();
		} else if (ConfigurationOptions.ES_BATCH_WRITE_RETRY_POLICY_NONE
				.equals(retryPolicyName)) {
			retryPolicyName = NoHttpRetryPolicy.class.getName();
		}

		retryPolicy = ObjectUtils.instantiate(retryPolicyName, settings);
		// Assume that the elasticsearch major version is the latest if the
		// version is not already present in the settings
	...
	BulkResponse processBulkResponse(Response response, TrackingBytesArray data) {
		...
		try {
			...
			for (Iterator<Map> iterator = r.readValues(parser); iterator
					.hasNext();) {
				...
				if (error != null && !error.isEmpty()) {
					if ((status != null && HttpStatus.canRetry(status))
						...
						if (errorMessagesSoFar < MAX_BULK_ERROR_MESSAGES) {
							// We don't want to spam the log with the same error
							// message 1000 times.
							// Chances are that the error message is the same
							// across all failed writes
							// and if it is not, then there are probably only a
							// few different errors which
							// this should pick up.
							errorMessageSample.add(error);
							errorMessagesSoFar++;
						}
					} else {
						String message = (status != null ? String.format(
					...
					esIdMapping
							.put(data.initialPosition(entryToDeletePosition),
									values);
			...
			int httpStatusToReport = entryToDeletePosition > 0 ? HttpStatus.SERVICE_UNAVAILABLE
					: HttpStatus.OK;
	...
	private String prettify(String error) {
		...
		String header = (invalidFragment != null ? "Invalid JSON fragment received["
				+ invalidFragment + "]"
				: "");
		return header + "[" + error + "]";
	}
	...
	public Map<String, Object> sampleForFields(String indexAndType,
		...
		Map<String, List<Map<String, Object>>> hits = parseContent(
				execute(GET, indexAndType + "/_search",
						new BytesArray(sb.toString())).body(), "hits");
		List<Map<String, Object>> docs = hits.get("hits");
		if (docs == null || docs.isEmpty()) {
			return Collections.emptyMap();
		}
		Map<String, Object> foundFields = docs.get(0);
		Map<String, Object> fieldInfo = (Map<String, Object>) foundFields
				.get("_source");
	...
	private void checkResponse(Request request, Response response) {
		if (response.hasFailed()) {
			...
			try {
				msg = extractError(this.<Map> parseContent(response.body(),
						null));
			...
			if (!StringUtils.hasText(msg)) {
				msg = String.format(
						"[%s] on [%s] failed; server[%s] returned [%s|%s:%s]",
	...
	public InputStream scroll(String scrollId) {
		...
		long start = network.transportStats().netTotalTime;
		try {
			// use post instead of get to avoid some weird encoding issues
			// (caused by the long URL)
			InputStream is = execute(POST,
					"_search/scroll?scroll=" + scrollKeepAlive.toString(),
					new BytesArray(scrollId)).body();
			stats.scrollTotal++;
			return is;
		} finally {
			stats.scrollTotalTime += network.transportStats().netTotalTime
					- start;
		}
	}
	...
	public boolean touch(String indexOrType) {
		if (!exists(indexOrType)) {
			...
			if (response.hasFailed()) {
				...
				if (StringUtils.hasText(msg)
						&& !msg.contains("IndexAlreadyExistsException")) {
	...
	public long count(String indexAndType, String shardId, QueryBuilder query) {
		return internalVersion.onOrAfter(EsMajorVersion.V_5_X) ? countInES5X(
				indexAndType, shardId, query) : countBeforeES5X(indexAndType,
				shardId, query);
	}

	private long countBeforeES5X(String indexAndType, String shardId,
			QueryBuilder query) {
	...
	private long countInES5X(String indexAndType, String shardId,
			QueryBuilder query) {
		StringBuilder uri = new StringBuilder(indexAndType);
	...
	public EsMajorVersion remoteEsVersion() {
		...
		if (result == null || !StringUtils.hasText(result.get("number"))) {
			throw new EsHadoopIllegalStateException(
					"Unable to retrieve elasticsearch version.");
		}
