...
public class NetworkClient implements StatsAware, Closeable {
	...
	public NetworkClient(Settings settings) {
		...
		// shuffle the list of nodes so in case of failures, the fallback is
		// spread
	...
	public Response execute(Request request) {
		...
		boolean newNode;
		do {
			SimpleRequest routedRequest = new SimpleRequest(request.method(),
					null, request.path(), request.params(), request.body());
			...
			try {
				...
				// issues with the SSL handshake, bail out instead of retry, for
				// security reasons
				...
				log.error(String.format("Node [%s] failed (%s); "
						+ (newNode ? "selected next node [" + currentNode + "]"
								: "no other nodes left - aborting..."), failed,
						ex.getMessage()));
